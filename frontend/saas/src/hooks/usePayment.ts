...
import { userQueryOptions, userBillingHistoryQueryOptions } from "../queries/userQueryOptions";
...
export default function usePayment() {
    ...
    const { mutate: mutateCancelSubscription } = useMutation({ mutationFn: () => cancelSubscription(),
                onSuccess: () => {
                    queryClient.invalidateQueries({queryKey: userBillingHistoryQueryOptions().queryKey})
                },
